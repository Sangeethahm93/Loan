...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.loan-applications.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="loan_purpose">{{ trans('cruds.loanApplication.fields.loan_purpose') }}</label>
                <input class="form-control {{ $errors->has('loan_purpose') ? 'is-invalid' : '' }}" type="text" name="loan_purpose" id="loan_purpose" value="{{ old('loan_purpose', '') }}" required>
                @if($errors->has('loan_purpose'))
                    <div class="invalid-feedback">
                        {{ $errors->first('loan_purpose') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.loanApplication.fields.loan_purpose_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="loan_tenure">{{ trans('cruds.loanApplication.fields.loan_tenure') }}</label>
                <input class="form-control {{ $errors->has('loan_tenure') ? 'is-invalid' : '' }}" type="text" name="loan_tenure" id="loan_tenure" value="{{ old('loan_tenure', '') }}" required>
                @if($errors->has('loan_tenure'))
                    <div class="invalid-feedback">
                        {{ $errors->first('loan_tenure') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.loanApplication.fields.loan_tenure_helper') }}</span>
            </div>
            <div class="form-group">
